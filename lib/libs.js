...
exports.startup = function startup(CmdInfo){
  ...
  rs = process.spawnSync(CmdInfo.StartCLI,['deployCC'],{
    cwd: path.resolve(CmdInfo.Path)
  });
  if (rs.status !=0) {
    console.log(rs);
...
exports.teardown = function teardown(CmdInfo){
  ...
  rs = process.spawnSync(CmdInfo.TapeCLI,['rm','tape']);
  if (rs.status !=0) {
    console.log(rs.stderr.toString());
  } else {
...
// docker run --name tape -e TAPE_LOGLEVEL=debug --network host -v $PWD:/config tape tape /config/config.yaml 500
// docker run --name tape -e TAPE_LOGLEVEL=debug --network net_test -v $PWD:/config tape tape /config/config.yaml 500
