...
router.get('/MaxMessageCount', function(req, res, next) {
    logger.info(req.query);
    const url = '/api/run?Path=.%2Ffabric-samples%2Ftest-network%2F&BatchTimeout=10&MaxMessageCount=10%2C20%2C40%2C80%2C160%2C320%2C640%2C1280&AbsoluteMaxBytes=20&PreferredMaxBytes=20480&CoolDown=5&PrepareCLI=.%2FprepareConfig.sh&StartCLI=.%2Fnetwork.sh&CCDeployCLI=.%2Fnetwork.sh&TapeCount=10000&ShutDownCLI=.%2Fnetwork.sh';
    if (req.query.DryRun) {
        res.redirect(url + '&DryRun=true');
    } else {
        res.redirect(url);
